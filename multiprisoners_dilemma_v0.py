# Version 0 preserves the minor offense sentence to one year, suitable for 2+ players
print("Welcome to Prisoner's Dilemma - Multiplayer Mode 0")
print("Here we extend the classic Prisoner's Dilemma to more players!")
print("Context:")
print(
    "n members of a gang have been arrested. The police possess evidence of a minor offense committed by the gang, enabling "
    "them to sentence each member to ONE year in prison. However, the police has made a bargain with each and every member: "
    "Your part in the minor offense will be pardoned if you confess to the major one. Your own confession will not "
    "incriminate yourself, but each confession from another member will add TWO years to your prison sentence."
)
print(
    "Put mathematically, a member's sentence with be 2 * number_of_other_confessions if they confess, and 1 + 2 * number_of_"
...
def choices_to_scores(choices):
    ...
    for choice in choices:  # calculates score based on choices
        if choice == "x":
            round_scores.append(-1 - 2 * num_comp)
        else:
